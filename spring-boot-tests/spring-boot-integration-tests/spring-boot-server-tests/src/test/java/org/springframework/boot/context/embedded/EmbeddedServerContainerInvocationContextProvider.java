...
class EmbeddedServerContainerInvocationContextProvider
	...
	private static final Set<String> CONTAINERS = new HashSet<>(
			Arrays.asList("jetty", "tomcat", "undertow"));
	...
	public Stream<TestTemplateInvocationContext> provideTestTemplateInvocationContexts(
		...
		return CONTAINERS.stream().map((container) -> new ApplicationBuilder(this.tempDir,
				annotation.packaging(), container)).flatMap((builder) -> {
					return Stream.of(annotation.launchers())
							.map((launcherClass) -> ReflectionUtils
									.newInstance(launcherClass, builder, buildOutput))
	...
	static class EmbeddedServletContainerInvocationContext
		...
		public EmbeddedServletContainerInvocationContext(String name,
		...
		public List<Extension> getAdditionalExtensions() {
			return Arrays.asList(this.launcher,
					new RestTemplateParameterResolver(this.launcher));
		}
		...
		public boolean supportsParameter(ParameterContext parameterContext,
			...
			if (parameterContext.getParameter().getType()
					.equals(AbstractApplicationLauncher.class)) {
				return true;
			}
			...
			rest.setUriTemplateHandler(new UriTemplateHandler() {
				...
				public URI expand(String uriTemplate, Object... uriVariables) {
					return URI.create("http://localhost:"
							+ RestTemplateParameterResolver.this.launcher.getHttpPort()
							+ uriTemplate);
				}
