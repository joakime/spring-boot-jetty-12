...
public class CachesEndpointWebIntegrationTests {
	...
	@WebEndpointTest
	public void allCaches(WebTestClient client) {
	...
	@WebEndpointTest
	public void namedCache(WebTestClient client) {
	...
	@WebEndpointTest
	public void namedCacheWithUnknownName(WebTestClient client) {
		client.get().uri("/actuator/caches/does-not-exist").exchange().expectStatus()
				.isNotFound();
	}

	@WebEndpointTest
	public void namedCacheWithNonUniqueName(WebTestClient client) {
		client.get().uri("/actuator/caches/a").exchange().expectStatus().isBadRequest();
	}

	@WebEndpointTest
	public void clearNamedCache(WebTestClient client, ApplicationContext context) {
	...
	@WebEndpointTest
	public void cleanNamedCacheWithUnknownName(WebTestClient client) {
		client.delete().uri("/actuator/caches/does-not-exist").exchange().expectStatus()
				.isNotFound();
	}

	@WebEndpointTest
	public void clearNamedCacheWithNonUniqueName(WebTestClient client) {
		client.get().uri("/actuator/caches/a").exchange().expectStatus().isBadRequest();
	}
