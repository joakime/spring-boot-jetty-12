...
In order to release logging resources when your application terminates, a shutdown hook is registered that will trigger log system cleanup when the JVM exits.
If your application has complex context hierarchies or is deployed as a war file, the shutdown hook may not meet your needs.
