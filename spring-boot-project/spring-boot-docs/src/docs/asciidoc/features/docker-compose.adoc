...
* `none` - Do not start up or shut down Docker Compose
* `start-up-only` - Start Docker Compose on application startup and leave it running
* `start-up-and-shut-down` - Start Docker Compose on application startup and shut it down on application shutdown

In addition you can use the configprop:spring.docker.compose.startup.command[] property to change if `docker compose up` or `docker compose start` is used.
The configprop:spring.docker.compose.shutdown.command[] allows you to configure if `docker compose down` or `docker compose stop` is used.
