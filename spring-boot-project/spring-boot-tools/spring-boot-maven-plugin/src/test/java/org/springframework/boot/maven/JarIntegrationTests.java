...
class JarIntegrationTests extends AbstractArchiveIntegrationTests {
	...
	void whenAClassifierIsConfiguredTheRepackagedJarHasAClassifierAndBothItAndTheOriginalAreInstalled() {
		new MavenBuild("jar-classifier-main").goals("install").execute((project) -> {
			assertThat(new File(project, "target/jar-classifier-main-0.0.1.BUILD-SNAPSHOT.jar.original")).doesNotExist();
			...
			assertThat(buildLog(project)).contains("Attaching repackaged archive " + repackaged + " with classifier test")
	...
	void whenBothJarsHaveTheSameClassifierAndAttachIsDisabledOnlyTheOriginalJarIsInstalled() {
		new MavenBuild("jar-classifier-source-attach-disabled").goals("install").execute((project) -> {
			File original = new File(project, "target/jar-classifier-source-attach-disabled-0.0.1.BUILD-SNAPSHOT-test.jar.original");
			assertThat(original).isFile();
			File repackaged = new File(project, "target/jar-classifier-source-attach-disabled-0.0.1.BUILD-SNAPSHOT-test.jar");
			assertThat(jar(repackaged)).hasEntryWithNameStartingWith("BOOT-INF/classes/");
	...
	void whenAJarIsExecutableItBeginsWithTheDefaultLaunchScript() {
		new MavenBuild("jar-executable").execute((project) -> {
			...
			assertThat(jar(repackaged)).hasEntryWithNameStartingWith("BOOT-INF/classes/");
			assertThat(launchScript(repackaged)).contains("Spring Boot Startup Script").contains("MyFullyExecutableJarName").contains("MyFullyExecutableJarDesc");
		});
	}
	...
	void whenAJarIsBuiltWithLibrariesWithConflictingNamesTheyAreMadeUniqueUsingTheirGroupIds() {
		new MavenBuild("jar-lib-name-conflict").execute((project) -> {
			...
			assertThat(jar(repackaged))
				...
				.hasEntryWithName("BOOT-INF/lib/org.springframework.boot.maven.it.another-acme-lib-0.0.1.BUILD-SNAPSHOT.jar");
		});
	}
	...
	void whenADependendencyHasTestScopeItIsNotIncludedInTheRepackagedJar() {
		new MavenBuild("jar-test-scope").execute((project) -> {
			...
			assertThat(jar(main)).doesNotHaveEntryWithNameStartingWith("BOOT-INF/lib/log4j").hasEntryWithNameStartingWith("BOOT-INF/lib/spring-");		
		});	
	}
	...
	void whenAProjectIsBuiltWithALayoutPropertyTheSpecifiedLayoutIsUsed() {
		new MavenBuild("jar-with-layout-property").goals("package", "-Dspring-boot.repackage.layout=ZIP").execute((project) -> {
			File main = new File(project, "target/jar-with-layout-property-0.0.1.BUILD-SNAPSHOT.jar");
			assertThat(jar(main)).manifest((manifest) -> {
				manifest.hasMainClass("org.springframework.boot.loader.PropertiesLauncher");
				manifest.hasStartClass("org.test.SampleApplication");
			});
