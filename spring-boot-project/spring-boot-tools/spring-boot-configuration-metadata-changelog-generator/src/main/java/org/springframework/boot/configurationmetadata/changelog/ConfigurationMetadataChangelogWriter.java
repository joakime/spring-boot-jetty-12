...
class ConfigurationMetadataChangelogWriter implements AutoCloseable {
	...
	void write(ConfigurationMetadataDiff diff) {
		this.out.append(String.format("Configuration property changes between `%s` and " + "`%s`%n", diff.getLeftName(),
				diff.getRightName()));
		this.out.append(System.lineSeparator());
		this.out.append(String.format("== Deprecated in `%s`%n", diff.getRightName()));
		...
		this.out.append(System.lineSeparator());
		this.out.append(String.format("== New in `%s`%n", diff.getRightName()));
		writeAddedProperties(differencesByType.get(Type.ADDED));
		this.out.append(System.lineSeparator());
		this.out.append(String.format("== Removed in `%s`%n", diff.getRightName()));
