...
class ConfigurationMetadataDiffTests {
	...
	void diffContainsDifferencesBetweenLeftAndRightInputs() {
		...
		assertThat(diff.getLeftName()).isEqualTo("1.0");
		assertThat(diff.getRightName()).isEqualTo("2.0");
		assertThat(diff.getDifferences()).hasSize(4);
		List<Difference> added = diff.getDifferences()
			.stream()
			.filter((difference) -> difference.type() == Type.ADDED)
			.collect(Collectors.toList());
		assertThat(added).hasSize(1);
		assertProperty(added.get(0).right(), "test.add", String.class, "new");
		List<Difference> deleted = diff.getDifferences()
			.stream()
		...
		List<Difference> deprecated = diff.getDifferences()
			.stream()
