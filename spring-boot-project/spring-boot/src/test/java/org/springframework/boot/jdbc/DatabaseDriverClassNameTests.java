...
public class DatabaseDriverClassNameTests {
	...
	@MethodSource
	void databaseClassIsOfRequiredType(DatabaseDriver driver, String className,
	...
	static Stream<? extends Arguments> databaseClassIsOfRequiredType() {
		return Stream.concat(
				argumentsForType(Driver.class, DatabaseDriver::getDriverClassName),
				argumentsForType(XADataSource.class,
						(databaseDriver) -> databaseDriver
								.getXaDataSourceClassName() != null,
	...
	private static Stream<? extends Arguments> argumentsForType(Class<?> clazz,
		...
		return Stream.of(DatabaseDriver.values())
				...
				.filter(predicate).map((databaseDriver) -> Arguments.of(databaseDriver,
						classNameExtractor.apply(databaseDriver), clazz));
	}
