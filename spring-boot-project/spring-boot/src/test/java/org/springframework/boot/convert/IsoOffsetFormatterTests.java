...
public class IsoOffsetFormatterTests {
	...
	public void convertShouldConvertStringToIsoDate(ConversionService conversionService) {
		OffsetDateTime now = OffsetDateTime.now();
		OffsetDateTime converted = conversionService.convert(
				DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(now), OffsetDateTime.class);
		assertThat(converted).isEqualTo(now);
	}

	@ConversionServiceTest
	public void convertShouldConvertIsoDateToString(ConversionService conversionService) {
		OffsetDateTime now = OffsetDateTime.now();
