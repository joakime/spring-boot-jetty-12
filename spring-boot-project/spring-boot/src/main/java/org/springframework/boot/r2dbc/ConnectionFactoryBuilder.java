...
public final class ConnectionFactoryBuilder {
	...
	static class OptionsCapableWrapper {
	...
	static class PoolingAwareOptionsCapableWrapper extends OptionsCapableWrapper {
		...
		private ConnectionPoolConfiguration connectionPoolConfiguration(ConnectionFactoryOptions delegateOptions,
				ConnectionFactory connectionFactory) {
			ConnectionPoolConfiguration.Builder builder = ConnectionPoolConfiguration.builder(connectionFactory);
			PropertyMapper map = PropertyMapper.get().alwaysApplyingWhenNonNull();
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.INITIAL_SIZE)).to(builder::initialSize);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.MAX_SIZE)).to(builder::maxSize);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.ACQUIRE_RETRY))
					.to(builder::acquireRetry);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.MAX_LIFE_TIME)).to(builder::maxLifeTime);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.MAX_ACQUIRE_TIME))
					.to(builder::maxAcquireTime);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.MAX_IDLE_TIME)).to(builder::maxIdleTime);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.MAX_CREATE_CONNECTION_TIME))
					.to(builder::maxCreateConnectionTime);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.POOL_NAME)).to(builder::name);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.REGISTER_JMX)).to(builder::registerJmx);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.VALIDATION_QUERY))
					.to(builder::validationQuery);
			map.from(delegateOptions.getValue(PoolingConnectionFactoryProvider.VALIDATION_DEPTH))
