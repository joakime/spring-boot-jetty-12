...
class DataSourceInitializationConfiguration {
	...
	private static DataSource determineDataSource(Supplier<DataSource> dataSource, String username, String password,
		...
		if (StringUtils.hasText(username) && StringUtils.hasText(password)) {
			return DataSourceBuilder.create(properties.getClassLoader())
					.driverClassName(properties.determineDriverClassName()).type(SimpleDriverDataSource.class)
					.url(properties.determineUrl()).username(username).password(password).build();
		}
	...
	// Compiler bug requires this to be fully-qualified
	...
	static class InitializationModeDataSourceScriptDatabaseInitializer extends DataSourceScriptDatabaseInitializer {
		...
		@Override
		protected void applyScripts(List<Resource> resources, boolean continueOnError, String separator,
				Charset encoding) {
			if (this.mode == DataSourceInitializationMode.ALWAYS || (this.mode == DataSourceInitializationMode.EMBEDDED
				...
				super.applyScripts(resources, continueOnError, separator, encoding);
			}
		}
	...
	static class DataSourceInitializationCondition extends SpringBootCondition {
		...
		public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
			...
			Set<String> configuredProperties = INITIALIZATION_PROPERTIES.stream().filter(environment::containsProperty)
			...
			return ConditionOutcome
					.match(message.found("configured property", "configured properties").items(configuredProperties));
		}
