...
public abstract class AbstractPropertiesConfigAdapterTests<P, A extends PropertiesConfigAdapter<P>> {
	...
	protected final void adapterOverridesAllConfigMethodsExcept(String... nonConfigMethods) {
		...
		Set<String> expectedConfigMethodNames = Arrays.stream(config.getDeclaredMethods()).filter(Method::isDefault)
				.filter(this::hasNoParameters).filter(this::isNotValidationMethod).filter(this::isNotDeprecated)
		...
		while (!Object.class.equals(currentClass)) {
			actualConfigMethodNames.addAll(Arrays.stream(currentClass.getDeclaredMethods()).map(Method::getName)
					.filter(expectedConfigMethodNames::contains).collect(Collectors.toList()));
